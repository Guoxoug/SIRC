...
def tf2th(conv_weights, dev="cuda"):
    """Possibly convert HWIO to OIHW."""
    if conv_weights.ndim == 4:
        conv_weights = conv_weights.transpose([3, 2, 0, 1])
    return torch.from_numpy(conv_weights).to(dev)
...
class PreActBottleneck(nn.Module):
    ...
    def load_from(self, weights, prefix='', dev="cuda"):
        convname = 'standardized_conv2d'
        with torch.no_grad():
            self.conv1.weight.copy_(
                tf2th(weights[f'{prefix}a/{convname}/kernel'], dev=dev))
            self.conv2.weight.copy_(
                tf2th(weights[f'{prefix}b/{convname}/kernel'], dev=dev))
            self.conv3.weight.copy_(
                tf2th(weights[f'{prefix}c/{convname}/kernel'], dev=dev))
            self.gn1.weight.copy_(
                tf2th(weights[f'{prefix}a/group_norm/gamma'], dev=dev))
            self.gn2.weight.copy_(
                tf2th(weights[f'{prefix}b/group_norm/gamma'], dev=dev))
            self.gn3.weight.copy_(
                tf2th(weights[f'{prefix}c/group_norm/gamma'], dev=dev))
            self.gn1.bias.copy_(tf2th(weights[f'{prefix}a/group_norm/beta'], dev=dev))
            self.gn2.bias.copy_(tf2th(weights[f'{prefix}b/group_norm/beta'], dev=dev))
            self.gn3.bias.copy_(tf2th(weights[f'{prefix}c/group_norm/beta'], dev=dev))
            if hasattr(self, 'downsample'):
                w = weights[f'{prefix}a/proj/{convname}/kernel']
                self.downsample.weight.copy_(tf2th(w, dev=dev))


class ResNetV2(nn.Module):
    """Implementation of Pre-activation (v2) ResNet mode."""

    def __init__(self, block_units, width_factor, num_classes=1000, num_block_open=-1):
        super().__init__()
    ...
    def intermediate_forward(
        self, x, layer_index=None, feature_threshold=torch.inf
    ):
        if layer_index == 'all':
            out_list = []
            out = self.root(x)
            ...
            out_list.append(out)
            out = self.before_head(out).clamp(max=feature_threshold)
            out_list.append(out)
            out = self.head(out)
    ...
    def forward(
        self, x, layer_index=None, 
        return_features=False, feature_threshold=torch.inf
    ):

        # to match with other models in repo
        if return_features:
            x, feature_list = self.intermediate_forward(
                x, layer_index="all", feature_threshold=feature_threshold
            )
            return x, feature_list[-1][..., 0, 0]
    ...
    def load_from(self, weights, prefix='resnet/', dev="cuda"):
        with torch.no_grad():
            self.root.conv.weight.copy_(
                tf2th(weights[f'{prefix}root_block/standardized_conv2d/kernel'], dev=dev))  # pylint: disable=line-too-long
            self.before_head.gn.weight.copy_(
                tf2th(weights[f'{prefix}group_norm/gamma'], dev=dev))
            self.before_head.gn.bias.copy_(
                tf2th(weights[f'{prefix}group_norm/beta'], dev=dev))

            self.head.conv.weight.copy_(
                tf2th(weights[f'{prefix}head/conv2d/kernel'], dev=dev))  # pylint: disable=line-too-long
            self.head.conv.bias.copy_(
                tf2th(weights[f'{prefix}head/conv2d/bias'], dev=dev))

            for bname, block in self.body.named_children():
                for uname, unit in block.named_children():
                    unit.load_from(weights, prefix=f'{prefix}{bname}/{uname}/', dev=dev)
